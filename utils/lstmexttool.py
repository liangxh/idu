# created by Theano Team
# imported by thlstm.py
...
def init_params(options, Wemb = None):
    ...
    # classifier
    params['U'] = 0.01 * numpy.random.randn(options['dim_proj'] + options['dim_sup'],
...
def adadelta(lr, tparams, grads, x, mask, xsup, y, cost):
    """
    ...
    f_grad_shared = theano.function([x, mask, xsup, y], cost, updates=zgup + rg2up,
...
def build_model(tparams, options):
    ...
    # xsup - lxh
    xsup = tensor.matrix('xsup', dtype = theano.config.floatX)
    softmax_input = tensor.concatenate([proj, xsup], axis = 1)

    pred = tensor.nnet.softmax(tensor.dot(softmax_input, tparams['U']) + tparams['b'])

    f_pred_prob = theano.function([x, mask, xsup], pred, name='f_pred_prob')
    f_pred = theano.function([x, mask, xsup], pred.argmax(axis=1), name='f_pred')
    ...
    return use_noise, x, mask, xsup, y, f_pred_prob, f_pred, cost
...
def pred_error(f_pred, prepare_data, data, iterator, verbose=False):
    ...
    for _, valid_index in iterator:
        ...
        xsup = [data[-1][t] for t in valid_index]
        preds = f_pred(x, mask, xsup)
